...
case $BLOCK_BUTTON in
  2) set locCode="ASI|BD|BD002|DHAKA" ;; # left click Shanghai, China
  3) set locCode="NAM|US|AL|35801" ;; # right click Huntsville, Alabama
esac
...
if( $values[0] eq "Showers"|| $values[0] eq "Thunderstorms"  || $values[0] eq "Mostly Cloudy w/ Showers" || $values[0] eq "Partly Sunny w/ Showers" || $values[0] eq "T-Storms"|| $values[0] eq "Mostly Cloudy w/ T-Storms"|| $values[0] eq "Partly Sunny w/ T-Storms"|| $values[0] eq "Rain")
{
#my $rain = "☂";
my $rain = "RAINY";
binmode(STDOUT, ":utf8");
