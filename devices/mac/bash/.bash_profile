...
# Macs have bash3 for which the bash-completion package doesn't include
# _init_completion. This is a minimal version of that function.
__exercism_init_completion()
{
    COMPREPLY=()
    _get_comp_words_by_ref "$@" cur prev words cword
}

# This function calls the exercism program to obtain the completion
# results and the directive.  It fills the 'out' and 'directive' vars.
__exercism_get_completion_results() {
    local requestComp lastParam lastChar args

    # Prepare the command to request completions for the program.
    # Calling ${words[0]} instead of directly exercism allows to handle aliases
    args=("${words[@]:1}")
    ...
    # When completing a flag with an = (e.g., exercism -n=<TAB>)
    # bash focuses on the part after the =, so we need to remove
    # the flag part from $cur
    if [[ ${cur} == -*=* ]]; then
        cur="${cur#*=}"
    ...
    __exercism_debug "Calling ${requestComp}"
    # Use eval to handle any environment variables and such
    out=$(eval "${requestComp}" 2>/dev/null)
    ...
    __exercism_debug "The completion directive is: ${directive}"
    __exercism_debug "The completions are: ${out}"
}

__exercism_process_completion_results() {
    local shellCompDirectiveError=1
    local shellCompDirectiveNoSpace=2
    local shellCompDirectiveNoFileComp=4
    local shellCompDirectiveFilterFileExt=8
    ...
    if (((directive & shellCompDirectiveError) != 0)); then
        # Error code.  No completion.
        __exercism_debug "Received error from custom completion go code"
        return
    else
        if (((directive & shellCompDirectiveNoSpace) != 0)); then
            if [[ $(type -t compopt) == builtin ]]; then
                __exercism_debug "Activating no space"
                compopt -o nospace
            else
                __exercism_debug "No space directive not supported in this version of bash"
            fi
        fi
        if (((directive & shellCompDirectiveKeepOrder) != 0)); then
            if [[ $(type -t compopt) == builtin ]]; then
                # no sort isn't supported for bash less than < 4.4
                if [[ ${BASH_VERSINFO[0]} -lt 4 || ( ${BASH_VERSINFO[0]} -eq 4 && ${BASH_VERSINFO[1]} -lt 4 ) ]]; then
                    __exercism_debug "No sort directive not supported in this version of bash"
                else
        ...
        if (((directive & shellCompDirectiveNoFileComp) != 0)); then
            if [[ $(type -t compopt) == builtin ]]; then
                __exercism_debug "Activating no file completion"
                compopt +o default
            else
                __exercism_debug "No file completion directive not supported in this version of bash"
            fi
        fi
    fi
    ...
    if (((directive & shellCompDirectiveFilterFileExt) != 0)); then
        ...
        __exercism_handle_completion_types
    fi

    __exercism_handle_special_char "$cur" :
    __exercism_handle_special_char "$cur" =
...
__exercism_handle_completion_types() {
    __exercism_debug "__exercism_handle_completion_types: COMP_TYPE is $COMP_TYPE"
    ...
    *)
        # Type: complete (normal completion)
        __exercism_handle_standard_completion_case
        ;;
    esac
}
...
__exercism_handle_standard_completion_case() {
    ...
    # If there is a single completion left, remove the description text
    if ((${#COMPREPLY[*]} == 1)); then
        __exercism_debug "COMPREPLY[0]: ${COMPREPLY[0]}"
        comp="${COMPREPLY[0]%%$tab*}"
        __exercism_debug "Removed description from single completion, which is now: ${comp}"
        COMPREPLY[0]=$comp
    else # Format the descriptions
        __exercism_format_comp_descriptions $longest
    fi
}

__exercism_handle_special_char()
{
    local comp="$1"
    local char=$2
    if [[ "$comp" == *${char}* && "$COMP_WORDBREAKS" == *${char}* ]]; then
...
__exercism_format_comp_descriptions()
{
    local tab=$'\t'
    local comp desc maxdesclength
    local longest=$1
    ...
    for ci in ${!COMPREPLY[*]}; do
        ...
        # Properly format the description string which follows a tab character if there is one
        if [[ "$comp" == *$tab* ]]; then
            __exercism_debug "Original comp: $comp"
            desc=${comp#*$tab}
            comp=${comp%%$tab*}
            ...
            COMPREPLY[ci]=$comp
            __exercism_debug "Final comp: $comp"
        fi
    done
}
...
__start_exercism()
{
    ...
    if declare -F _init_completion >/dev/null 2>&1; then
        ...
        __exercism_init_completion -n =: || return
    fi

    __exercism_debug
    __exercism_debug "========= starting completion logic =========="
    __exercism_debug "cur is ${cur}, words[*] is ${words[*]}, #words[@] is ${#words[@]}, cword is $cword"
...
if [[ $(type -t compopt) = "builtin" ]]; then
    complete -o default -F __start_exercism exercism
else
    complete -o default -o nospace -F __start_exercism exercism
fi
